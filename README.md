# KKL observer for nonlinear systems with inputs

Our goal is to synthesize a numerical nonlinear Luenberger observer (KKL observer) for a damped pendulum with state $x\in \mathbb{R}^2$, measured position $x_1$ and sinusoidal excitation $u$. Because the input is not known in advance, we design the observer for a class of inputs generated by an exosystem
$$
\dot{w} = s(w) \quad , \quad u=w_1
$$
with $w\in \mathbb{R}^3$ where $(w_1,w_2)$ is the state of a linear oscillator with constant frequency $w_3$. In other words, we design a functional KKL observer for the extended system $(x,w)$ reconstructing $x$ from the knowledge of $x_1$ and the input $u$.  Neural networks are used to learn the transformation $z=T(x,w)$ and its inverse $x=\tau(z)$. 
